/**
 * A-Zent IT保守サブスク - Cloudflare Workers メインエントリ
 *
 * ルーティング:
 *   POST /webhook/line   LINE Webhookを受信(顧客からの症状問い合わせ)
 *   POST /webhook/vendor LINE Webhookを受信(業者からの受注・報告)
 *   GET  /health         動作確認用
 *
 * 環境変数(Cloudflare Dashboard の Secrets で設定):
 *   LINE_CHANNEL_ACCESS_TOKEN  LINE公式アカウントのトークン
 *   LINE_CHANNEL_SECRET        LINE署名検証用シークレット
 *   AZENT_ADMIN_LINE_ID        全業者未応答時のエスカレーション先(自分のLINE ID)
 *   APPROVAL_THRESHOLD         自動承認の上限額(デフォルト10000)
 */
...
async function handleCustomerWebhook(request, env) {
  try {
    ...
    for (const event of events) {
      ...
      if (!company) {
        await line.reply(event.replyToken, '登録されていないLINEアカウントです。担当者にご確認ください。');
        continue;
      }
